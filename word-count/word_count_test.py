...
class WordCountTest(unittest.TestCase):
    ...
    def test_count_one_of_each_word(self):
        self.assertEqual(
            count_words("one of each"), {"one": 1, "of": 1, "each": 1}
        )
    ...
    def test_handles_cramped_lists(self):
        self.assertEqual(
            count_words("one,two,three"), {"one": 1, "two": 1, "three": 1}
        )
    ...
    def test_normalize_case(self):
        self.assertEqual(
            count_words("go Go GO Stop stop"), {"go": 3, "stop": 2}
        )
    ...
    def test_tabs(self):
        self.assertEqual(
            count_words(
                "rah rah ah ah ah	roma roma ma	ga ga oh la la	want your "
                "bad romance"
            ),
            {
                "rah": 2,
