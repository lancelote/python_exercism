...
class DndCharacterTest(unittest.TestCase):
    ...
    def test_random_ability_is_within_range(self):
        ...
        self.assertIs(3 <= score <= 18, True)

    def test_random_character_is_valid(self):
        char = Character()
        self.assertIs(3 <= char.strength <= 18, True)
        self.assertIs(3 <= char.dexterity <= 18, True)
        self.assertIs(3 <= char.constitution <= 18, True)
        self.assertIs(3 <= char.intelligence <= 18, True)
        self.assertIs(3 <= char.wisdom <= 18, True)
        self.assertIs(3 <= char.charisma <= 18, True)
